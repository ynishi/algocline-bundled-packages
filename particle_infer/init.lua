...
---
--- Implements the paper's §3.1 Algorithm 1 single-chain Particle Filter
--- under a State-Space-Model formulation of LLM generation. N rollouts
--- (particles) are advanced one step at a time; a caller-injected
--- Process Reward Model (PRM) scores each step; the particles are
--- resampled at every step via `softmax(w)`; an optional Outcome Reward
--- Model (ORM) picks the final answer.
---
--- ## Usage
---
--- ```lua
