...
---
--- Given N candidate answers and a D-dimensional rubric, the package
--- evaluates `(candidate × dimension)` pairs incrementally and kills
--- candidates whose anytime-valid upper confidence bound drops below the
--- best surviving candidate's lower confidence bound.
---
--- ## Usage
---
--- ```lua
        ...
        run = {
            ...
            result = T.shape({
                ...
                ranking         = T.array_of(T.shape({
                    ...
                    n      = T.number,
                    mean   = T.number,
                    lcb    = T.number:describe("Lower confidence bound on mean"),
                    ucb    = T.number:describe("Upper confidence bound on mean"),
                    radius = T.number:describe("Confidence radius (half-width of CS interval)"),
                    v_hat  = T.number:describe("Running variance estimate V̂"),
                ...
                rounds          = T.array_of(T.shape({
                    ...
                    score          = T.number,
                    n_after        = T.number,
                    v_hat_after    = T.number:describe("V̂ after this update"),
                    mean_after     = T.number:describe("Empirical mean after this update"),
