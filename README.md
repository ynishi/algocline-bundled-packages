...
| **[dmad](dmad/)** | Multi-Agent Debate (paper-explicit). N=3 parallel agents propose initial answers, then for R=2 debate rounds each agent revises after seeing the other N-1 agents' previous-round responses. Majority vote (first-wins tie-break) on `\boxed{answer}` extractions. Total N·(R+1)=9 LLM calls default. Distinct methodology from `hegelian` (Abdali 2025's single-thread asymmetric dialectic) | Du et al. (arXiv:2305.14325, 2023) |
...
| **[speculative_rejection](speculative_rejection/)** | Iterative reward-pruned Best-of-N. Adapts Sun 2024 Algorithm 1 (token-level speculative rejection) to `alc.llm` call granularity: each round runs one `alc.llm_batch` generation/extension pass + one `alc.llm` reward scoring pass, then rejects the bottom α fraction (default α=0.5, paper §4.1 canonical). Trades more sequential calls (7 vs 2 at defaults) for fewer generation tokens spent on eventual losers — NOT a strict Pareto improvement over `verify_select`; win when generations are long and losers can be identified early. Adaptation gap: paper's ~24x speedup depends on shared token-level forward pass which does not carry over to call granularity | Sun et al. "Fast Best-of-N Decoding via Speculative Rejection" (NeurIPS 2024, arXiv:2410.20290) |
...
| **[moa](moa/)** | Mixture-of-Agents (paper-explicit). L=3 layers × n=6 proposers per layer (Wang §3 main config; MoA-Lite L=2 for cost). Each layer applies the Aggregate-and-Synthesize prompt (Table 1; Lua string literal identical to the paper modulo `%s` substituting `{}`) — `y_i = ⊕[A_{i,j}(x_i)] + x_1`. Caller supplies `proposers` (multi-model PATH; Wang §3 main config) or `personas` (single-model rotation PATH; outside Wang §3's multi-model setup, X alt path). Total L·(n+1) LLM calls | Wang et al. (arXiv:2406.04692, 2024) |
| **[reconcile](reconcile/)** | ReConcile (paper-explicit) — round-table consensus with confidence-weighted voting. N=3 agents × R=3 max rounds (Chen §3). Each round: every agent emits (answer, explanation, confidence ∈ [0,1]); agents see others' triples in the next round; consensus check after each round triggers early stop. Final vote uses §B.5 5-bucket calibrated weights {0.1, 0.3, 0.5, 0.8, 1.0} (L; Lua transcription of repo `utils.py::trans_confidence` — same boundary values, same weights, top-to-bottom first-match) applied to §4 argmax formula `â = arg max_a Σ_i f(p_i)·𝟙(a_i=a)`. Caller supplies `agents` (diverse-LLM PATH; Chen §3 main config) or `personas` (single-model rotation PATH; X alt path). Total N to N·(R+1) LLM calls | Chen, Saha & Bansal (arXiv:2309.13007, 2023) |
...
| **[debate](debate/)** | Adversarial two-debater protocol with a terminal judge verdict. Two debaters argue opposing positions (`position_a` / `position_b`) for R rounds (default R=3, Khan 2024 §3 Table 2 canonical), each seeing the prior transcript. A single judge then emits `WINNER: A/B` + rationale under a truthfulness rubric. `alc.llm` call budget = 2R + 1 (7 at defaults), sequential (not batchable — each debater sees prior turns). Distinct from `panel` (deliberative multi-role synthesis) / `dissent` (critique-of-draft asymmetry) / `triad` (three mutually critical perspectives without a fixed winner) — this enforces symmetric pro/con exchange with a forced WINNER decision | Khan et al. "Debating with More Persuasive LLMs Leads to More Truthful Answers" (ICML 2024 Best Paper, arXiv:2402.06782) |
