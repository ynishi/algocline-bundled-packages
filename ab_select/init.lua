        ...
        run = {
            ...
            result = T.shape({
                ...
                ranking         = T.array_of(T.shape({
                    index          = T.number,
                    posterior_mean = T.number:describe("Beta-posterior mean α/(α+β)"),
                    alpha          = T.number:describe("Beta distribution α shape parameter"),
                    beta           = T.number:describe("Beta distribution β shape parameter"),
                    evaluations    = T.table:describe("Sparse map level(number) -> raw score"),
                    n_evals        = T.number:describe("Evaluation count for this candidate"),
                ...
                rounds          = T.array_of(T.shape({
                    ...
                    level_name  = T.string,
                    score       = T.number,
                    score_norm  = T.number:describe("Normalized score in [0, 1]"),
                    alpha       = T.number:describe("Beta α after this update"),
                    beta        = T.number:describe("Beta β after this update"),
                    theta_pick  = T.number:describe("Thompson-sampled θ used for this pick"),
                    cost        = T.number,
