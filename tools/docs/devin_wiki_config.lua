    ...
    repo_notes = {
        {
            content =
                ...
                "package's `init.lua` docstring plus its `M.meta` table. " ..
                "When generating wiki content, prefer these canonical " ..
                "narratives over re-deriving from the source files.",
        },
        {
            content =
                "Each package declares `M.meta.input_shape` (and often " ..
                "`M.meta.result_shape`) using the `alc_shapes.t` " ..
                "combinator DSL. This is the single source of truth for " ..
                "a package's I/O contract — the Parameters table in the " ..
                "narrative is a projection of that DSL, not separate " ..
                "documentation. Do not contradict the narrative's " ..
                "Parameters / result_shape with freshly-derived schemas.",
        },
        {
            content =
                "Strategies are invoked through `alc.run(ctx)` where " ..
                "`ctx` is a plain Lua table shaped by the package's " ..
                "`M.meta.input_shape`. An `alc.llm(prompt)` call inside " ..
                "a strategy pauses execution and resumes when the host " ..
                "provides the completion via `alc_continue`. Keep wiki " ..
                "pages aligned with this pause/resume execution model.",
        },
    },
