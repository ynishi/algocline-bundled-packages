...
---
--- Given N candidate answers and a D-dimensional rubric, evaluates
--- `(candidate × dimension)` pairs in block order (one rubric dimension
--- at a time across all alive candidates), maintains a rank matrix, and
--- eliminates candidates whose mean rank is significantly worse than the
--- best survivor by a Friedman + Nemenyi post-hoc test.
---
--- ## Usage
---
--- ```lua
        ...
        run = {
            ...
            result = T.shape({
                ...
                ranking         = T.array_of(T.shape({
                    ...
                    n         = T.number,
                    mean      = T.number,
                    mean_rank = T.number:is_optional():describe("Mean Friedman rank across blocks"),
                ...
                kill_events     = T.array_of(T.shape({
                    ...
                    n              = T.number,
                    mean           = T.number,
                    rank_sum       = T.number:describe("Friedman rank sum over observed blocks"),
                    best_rank_sum  = T.number:describe("Minimum rank sum across alive candidates"),
                    best_candidate = T.number:describe("Candidate index holding best_rank_sum"),
                    q              = T.number:describe("Friedman Q statistic"),
                    chi2_critical  = T.number:describe("χ² critical value at effective α"),
                    crit_diff      = T.number:describe("Nemenyi critical-difference bound on rank_sum gap"),
                    blocks_used    = T.number:describe("Blocks observed at this kill event"),
