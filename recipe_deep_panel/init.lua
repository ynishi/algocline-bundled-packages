...
M.caveats = {
    ...
    "Slot verification runs at the Frame boundary on every "
        .. "ab_mcts.run return. ab_mcts echoes _flow_token / _flow_slot "
        .. "by convention (see contract.md) but third-party pkg may not. "
        .. "For non-opt-in pkg flow.token_verify is fail-open: a missing "
        .. "echo passes verification, only a MISMATCHED echo fails"
        .. " → ab_mcts is opt-in (echoes). Swapping in a non-opt-in "
        .. "reasoning pkg silently weakens the integrity guarantee to "
        .. "'no cross-branch mis-routing is detectable'.",
