...
describe("tools.docs.extract.build_pkg_info (spec.entries.run.result)", function()
    it("wraps a string spec.entries.run.result as a T.ref schema", function()
        -- Write a temp pkg with spec.entries.run.result = "my.Result"
        ...
        f:write('category="c" }\n')
        f:write('M.spec = { entries = { run = { result = "my.Result" } } }\n')
    ...
    it("passes a T.shape spec.entries.run.result through as-is", function()
        local dir = os.tmpname()
        ...
        f:write('category="c" }\n')
        f:write('M.spec = { entries = { run = { result = T.shape({')
    ...
    it("rejects a non-string, non-schema spec.entries.run.result", function()
        local dir = os.tmpname()
        ...
        f:write('category="c" }\n')
        f:write('M.spec = { entries = { run = { result = 42 } } }\n')
        f:write('function M.run(c) return c end\nreturn M\n')
        f:close()
        local saved = package.path
        package.path = dir .. "/?.lua;" .. dir .. "/?/init.lua;" .. saved
